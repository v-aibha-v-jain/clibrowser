(function () {
  ...
  function getCustomBookmarks() {
    try { const s = localStorage.getItem('customBookmarks'); return s ? JSON.parse(s) : {}; } catch { return {}; }
  }
  function saveCustomBookmarks(b) { localStorage.setItem('customBookmarks', JSON.stringify(b || {})); }

  function getCustomFavorites() {
    try { const s = localStorage.getItem('customFavorites'); return s ? JSON.parse(s) : []; } catch { return []; }
  }
  function saveCustomFavorites(f) { localStorage.setItem('customFavorites', JSON.stringify(f || [])); }

  function getNotes() {
    try { const s = localStorage.getItem('notes'); return s ? JSON.parse(s) : []; } catch { return []; }
  }
  ...
  function handleCommand(line) {
    if (!line) { createCommandLine(); return; }
    ...
    if (cmd === 'mkbm') {
      ...
      if (args.length === 1) {
        ...
        getCurrentTabUrl((url) => {
          if (!url) return appendOutput('Could not get current tab URL (permission missing)', 'error');
    ...
    if (cmd === 'note') {
      ...
      if (!text) { appendOutput('Usage: note <text>'); createCommandLine(); return; }
